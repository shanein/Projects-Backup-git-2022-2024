...
class IA :
    ...
    def refresh_trackers(self, frame):
        # print(f"update_trackers")
        new_trackers = {}
        for person_id, data in self.face_trackers.items():
            tracker = data['tracker']
            ok, bbox = tracker.update(frame)
            x, y, w, h = bbox
            # Vérifiez que le tracker est dans les limites de l'image
    ...
    def initializeIADetection(self):
        ...
        if not ret:
            ...
            self.face_trackers, self.unique_people_counter = self.initialize_trackers(frame, faces, self.face_trackers, self.unique_people_counter)
...
def main():
    ...
    while True:
        ...
        if ia.frame_count % ia.prediction_interval == 0:
        ...
        print('print final boucle du resultats des viewers actifs')
